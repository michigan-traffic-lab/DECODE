...
class ErMotionTransformer(BaseModel):

    def __init__(self, config):
        super(ErMotionTransformer, self).__init__(config)
        self.config = config
        ...
        if self.model_cfg.method.get('strategy_name') == 'er' and self.model_cfg.domain.get('last_er_model_path', None) is not None:
            checkpoint_model = torch.load(self.model_cfg.domain.last_er_model_path)
            self.load_state_dict(checkpoint_model['state_dict'])
            print("loading model from", self.model_cfg.domain.last_er_model_path)
            
        elif self.model_cfg.method.get('pretrained_model_path', None) is not None:
            # load pre-trained model parameters 
            ckpt = torch.load(self.model_cfg.method.pretrained_model_path)
            self.load_state_dict(ckpt['state_dict'])
            print("loading model from", self.model_cfg.method.pretrained_model_path)
        else:
            raise ValueError("Please provide a valid strategy_name and model_path")
        # create an experience replay buffer
        self.buffer = Buffer(self.config)

    def forward(self, batch):
        real_batch_size = batch['batch_size']
        if not self.buffer.is_empty() and self.training:
            batch = self.buffer.add_to_batch(batch, device=self.device)
        ...
        if not self.buffer.is_empty() and self.training:
            batch, output = self.buffer.restore_batch(batch, output, real_batch_size)
        return output, loss
...
class Buffer:
    """
    The memory buffer of rehearsal method. Adapted from: https://github.com/aimagelab/mammoth/blob/master/utils/buffer.py#L252
    """
    ...
    examples: List[dict]  # the list of examples in the buffer

    def __init__(self, cfgs):
        # make a copy of the config
        self.cfgs = deepcopy(cfgs)
        self.buffer_size = self.cfgs.method.replay_buffer_size
        self.mini_batch_size = self.cfgs.method.replay_mini_batch_size
        self.num_seen_examples = 0
        self.examples = []
        self.sample_selection_fn = ReservoirSampling(self.buffer_size)
        buffer_data_paths = self.cfgs.domain.er_buffer_data_paths
        self.collate_fn = None
        for data_path in buffer_data_paths:
            self.cfgs['train_data_path'] = [data_path]
            self.cfgs['max_data_num'] = [1000]
            dataset = MTRDataset(self.cfgs, is_validation=False)
            if self.collate_fn is None:
                self.collate_fn = dataset.collate_fn
            self.fill_buffer(dataset)
    ...
    def add_data(self, examples):
        ...
        for i in range(len(examples)):
            ...
            if index >= 0:
                if len(self.examples) <= index:
                    self.examples.append(examples[i])
                else:
                    self.examples[index] = examples[i]
    
    def get_data(self, size, device):
        num_avail_samples = len(self.examples)
        num_avail_samples = min(self.num_seen_examples, num_avail_samples)
        if size > min(num_avail_samples, len(self.examples)):
            size = min(num_avail_samples, len(self.examples))

        choice = np.random.choice(num_avail_samples, size=size, replace=False)
        selected_samples = [[self.examples[i]] for i in choice]
        return self.collate_fn(selected_samples)

    def add_to_batch(self, batch, device='cuda'):
        batch_to_add = self.get_data(self.mini_batch_size, device)
        for key, val in batch_to_add['input_dict'].items():
            if type(val) == torch.Tensor:
                batch['input_dict'][key] = torch.cat((batch['input_dict'][key], val.to(device)), dim=0)
            elif type(val) == list:
                batch['input_dict'][key] += val
            else:
                batch['input_dict'][key] = np.concatenate((batch['input_dict'][key], val), axis=0)
        batch['batch_size'] += batch_to_add['batch_size']
        batch['batch_sample_count'] += batch_to_add['batch_sample_count']
        return batch
    
    def restore_batch(self, batch, output, real_batch_size):      
        for key, val in batch['input_dict'].items():
            batch['input_dict'][key] = val[:real_batch_size]
        for key, val in output.items():
            output[key] = val[:real_batch_size]
        batch['batch_size'] = real_batch_size
        batch['batch_sample_count'] = real_batch_size
        return batch, output