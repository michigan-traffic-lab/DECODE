...
class EwcMotionTransformer(BaseModel):

    def __init__(self, config):
        super(EwcMotionTransformer, self).__init__(config)
        self.config = config
        ...
        self.importance = self.model_cfg.method.ewc_importance
        # load previous model parameters
        if self.model_cfg.domain.get('last_ewc_model_path', None) is not None:
            checkpoint_model = torch.load(self.model_cfg.domain.last_ewc_model_path)
            self.load_state_dict(checkpoint_model['state_dict'])
            # freeze the context encoder
            for param in self.context_encoder.parameters():
                param.requires_grad = False
            print("loading model from", self.model_cfg.domain.last_ewc_model_path)
            self.fisher = {n: torch.tensor(p).cuda() for n, p in checkpoint_model['fisher_info'].items()}
            self.params = {n: torch.tensor(p).cuda() for n, p in checkpoint_model['optimal_params'].items()}
        else:
            ...
            self.params = {n: p.clone().detach().cuda() for n, p in self.named_parameters() if p.requires_grad}
            self.fisher = {n: torch.zeros_like(p).cuda() for n, p in self.named_parameters() if p.requires_grad}
    ...
    def compute_fisher_matrix(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            self.zero_grad()
            _, loss = self.forward(batch, ewc=False)
            loss.backward()
            data_count += batch['batch_size']
            for n, p in self.named_parameters():
                if p.grad is not None:
                    fisher_dict[n] += p.grad.pow(2)
        # Average the Fisher information by number of datapoints
        for n in fisher_dict:
            fisher_dict[n] /= data_count
        
        for n in self.fisher:
            fisher_dict[n] = fisher_dict[n] + self.fisher[n] * 0.8
        return fisher_dict
    
    def on_train_start(self) -> None:
        # put self.params and self.fisher on the same device as the model
        self.params = {n: p.to(self.device) for n, p in self.params.items()}
        self.fisher = {n: p.to(self.device) for n, p in self.fisher.items()}

    def forward(self, batch, ewc=True):
        ...
        loss = self.get_loss(ewc)
        return output, loss

    def get_loss(self, ewc=True):
        loss, tb_dict, disp_dict = self.motion_decoder.get_loss()
        if self.training_domain_id > 0 and ewc:
    ...
    def compute_ewc_loss(self):
        ...
        for n, p in self.named_parameters():
            if p.requires_grad:
                ewc_loss += self.importance * (self.fisher[n] * (p - self.params[n]).pow(2)).sum()
        return ewc_loss
    ...
    def on_save_checkpoint(self, checkpoint):
        if self.current_epoch == self.model_cfg.method.max_epochs - 1:
            self.params = {n: p.clone().detach() for n, p in self.named_parameters() if p.requires_grad}
            self.fisher = self.compute_fisher_matrix(self.train_loader)
        # Save Fisher information and optimal parameters
        checkpoint['fisher_info'] = {n: p.cpu().numpy() for n, p in self.fisher.items()}
        checkpoint['optimal_params'] = {n: p.cpu().numpy() for n, p in self.params.items()}
...
class MTRDecoder(nn.Module):
    def __init__(self, in_channels, config):
        ...
        temp_layer = build_mlps(c_in=self.d_model * 2 + map_d_model, mlp_channels=[self.d_model, self.d_model], without_norm=True,
                                ret_before_act=True)
    ...
    def build_dense_future_prediction_layers(self, hidden_dim, num_future_frames):
        ...
        self.dense_future_head = build_mlps(
            c_in=hidden_dim * 2,
            mlp_channels=[hidden_dim, hidden_dim, num_future_frames * 7], ret_before_act=True,without_norm=True
        )
    ...
    def build_motion_query(self, d_model, use_place_holder=False):
        ...
        if use_place_holder:
            ...
            intention_query_mlps = build_mlps(
                c_in=d_model, mlp_channels=[d_model, d_model], ret_before_act=True,without_norm=True
            )
    ...
    def build_motion_head(self, in_channels, hidden_size, num_decoder_layers):
        motion_reg_head = build_mlps(
            c_in=in_channels,
            mlp_channels=[hidden_size, hidden_size, self.num_future_frames * 7], ret_before_act=True,without_norm=True
        )
        motion_cls_head = build_mlps(
            c_in=in_channels,
            mlp_channels=[hidden_size, hidden_size, 1], ret_before_act=True,without_norm=True
        )
