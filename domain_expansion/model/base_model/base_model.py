...
class BaseModel(pl.LightningModule):
    ...
    @rank_zero_only
    def log_to_wandb(self, batch, prediction):
        image = visualization.visualize_prediction(batch, prediction)
        wandb.log({"prediction": [wandb.Image(image)]})
        image.close()

    def compute_metrics(self, batch, batch_idx, prediction, status='train'):
        ...
        bpd = prediction['bpd'].detach().cpu().numpy()
        loss_dict = {
            'bpd': bpd.astype(np.float32),
            'minADE6': minade,
            'minFDE6': minfde,
    ...
    def log_info(self, batch, batch_idx, prediction, status='train', record_dataloader=True):
        ## logging
        # Split based on dataset
        inputs = batch['input_dict']
        gt_traj = inputs['center_gt_trajs'].unsqueeze(1)  # .transpose(0, 1).unsqueeze(0)
        ...
        loss_dict = {
            ...
            'miss_rate': miss_rate.astype(np.float32),
            'brier_fde': brier_fde}
        if 'bpd' in prediction:
            bpd = prediction['bpd'].detach().cpu().numpy()
            loss_dict['bpd'] = bpd.astype(np.float32)
        ...
        for dataset_name in unique_dataset_names:
            ...
            for key in loss_dict.keys():
                dataset_new_dict[dataset_name + '/' + key] = loss_dict[key][batch_idx_for_this_dataset]
        dataset_loss_dict = {key: np.mean(value) for key, value in dataset_new_dict.items()}
        dataset_size_dict = {key: len(value) for key, value in dataset_new_dict.items()}
        ...
        for k, v in dataset_loss_dict.items():
            self.log(status + "/" + k, v, on_step=False, on_epoch=True, sync_dist=True, batch_size=dataset_size_dict[k], add_dataloader_idx=False)
        if record_dataloader:
            for k, v in loss_dict.items():
                self.log(status + "/" + k, v, on_step=False, on_epoch=True, sync_dist=True, batch_size=size_dict[k], add_dataloader_idx=False)

            if status == 'val' and batch_idx == 0 and not self.config.debug:
                self.log_to_wandb(batch, prediction)
                # img = visualization.visualize_prediction(batch, prediction)
                # wandb.log({"prediction": [wandb.Image(img)]})

        return

    def log_hidden(self, forward_ret_dict, batch_idx, status='train'):
        return
        # log the hidden features 
        wandb.log({status + '/query_content5': wandb.Histogram(forward_ret_dict['query_content5'].cpu().detach().numpy())})
        wandb.log({status + '/query_feature5': wandb.Histogram(forward_ret_dict['query_feature5'].cpu().detach().numpy())})
