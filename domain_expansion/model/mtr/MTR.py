...
class MotionTransformer(BaseModel):
    ...
    def __init__(self, config):
        ...
        if self.model_cfg.method.get('strategy_name') == 'no_rehearsal' and self.model_cfg.domain.get('no_rehearsal_model_path', None) is not None:
            checkpoint_model = torch.load(self.model_cfg.domain.no_rehearsal_model_path)
            self.load_state_dict(checkpoint_model['state_dict'])
            print("loading model from", self.model_cfg.domain.no_rehearsal_model_path)
